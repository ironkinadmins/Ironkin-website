...
export async function getConfig(env){const raw=await hybridKv(env, "drops").get(CONFIG_KEY);if(!raw)return defaultConfig();try{const p=JSON.parse(raw),b=defaultConfig();for(const t of ["team1","team2"]){b[t]={...b[t],...(p[t]||{}),name:cleanName(p[t]?.name,b[t].name),iterations:Math.min(ITERATIONS,Math.max(1,Number(p[t]?.iterations)||ITERATIONS)),version:Math.max(0,Number(p[t]?.version)||0)};}b.updatedAt=p.updatedAt||b.updatedAt;return b;}catch{return defaultConfig();}}
export async function saveConfig(env,c){c.updatedAt=new Date().toISOString();await hybridKv(env, "drops").put(CONFIG_KEY,JSON.stringify(c));}
...
export async function rateStatus(request,env,team){const key=`bingo:team-login:${team}:${ip(request)}`;const n=Number(await hybridKv(env, "drops").get(key)||0);return{key,allowed:n<MAX_ATTEMPTS};}
export async function failRate(env,key){const n=Number(await hybridKv(env, "drops").get(key)||0)+1;await hybridKv(env, "drops").put(key,String(n),{expirationTtl:WINDOW});}
export async function clearRate(env,key){await hybridKv(env, "drops").delete(key);}
