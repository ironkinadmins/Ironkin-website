...
export async function onRequestPost({request,env}){
  ...
  const open=challenge.opensAt?new Date(challenge.opensAt).getTime():-Infinity,close=challenge.closesAt?new Date(challenge.closesAt).getTime():Infinity,duration=challenge.durationMode==="week"?0:Math.max(1,Number(challenge.durationMinutes||60))*60000;
  if(scheduled.getTime()<open || (duration && scheduled.getTime()+duration>close)) return Response.json({error:"That slot falls outside the challenge window."},{status:409});
