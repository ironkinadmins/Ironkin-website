...
function cleanAttack(a){return{id:String(a?.id||""),attackingTeam:a?.attackingTeam,defendingTeam:a?.defendingTeam,targetIndex:Number(a?.targetIndex),result:a?.result==="hit"?"hit":"miss",at:String(a?.at||"")};}
function completedTileCount(attacks,team){const resolved=new Set();for(const attack of (Array.isArray(attacks)?attacks:[])){if(attack?.attackingTeam!==team)continue;if(attack?.result!=="hit"&&attack?.result!=="miss")continue;const tileIndex=Number(attack?.targetIndex);if(Number.isInteger(tileIndex)&&tileIndex>=0&&tileIndex<100)resolved.add(tileIndex);}return resolved.size;}
...
export async function onRequestGet({request,env}){const access=await getTeamSession(request,env);if(!access)return Response.json({error:"Team password required."},{status:401,headers:noStore});const user=await requireBingoTeam(request,env,access);if(!user.ok)return Response.json({error:user.error},{status:user.status,headers:noStore});const own=boardTeam(access),opponent=own==="ember"?"ash":"ember";const raw=await hybridKv(env, "drops").get("bingo:state:v2");if(!raw)return Response.json({error:"The Bingo board has not been created yet."},{status:404,headers:noStore});const state=enforceStateIntegrity(JSON.parse(raw)),config=await getConfig(env);const ownTeam=state.teams?.[own]||{},opponentTeam=state.teams?.[opponent]||{};const attacks=(Array.isArray(state.attacks)?state.attacks:[]).filter(a=>a.attackingTeam===own||a.defendingTeam===own).map(cleanAttack);return Response.json({version:1,accessTeam:access,viewerTeam:own,isStaff:Boolean(user.isStaff),size:10,phase:state.phase||"setup",locked:Boolean(state.locked),updatedAt:state.updatedAt||"",tiles:(Array.isArray(state.tiles)?state.tiles:[]).slice(0,100).map((t,i)=>cleanTile(t,i,own)),ownTeam:{key:own,name:config[access].name,captain:String(ownTeam.captain||""),fleetConfirmed:Boolean(ownTeam.fleetConfirmed),ships:[]},opponent:{key:opponent,name:config[access==="team1"?"team2":"team1"].name,captain:String(opponentTeam.captain||"")},fleetSummary:{[own]:cleanFleetSummary(ownTeam,own,state.attacks),[opponent]:cleanFleetSummary(opponentTeam,opponent,state.attacks)},completedTiles:{[own]:completedTileCount(state.attacks,own),[opponent]:completedTileCount(state.attacks,opponent)},attacks,proofs:(Array.isArray(state.proofs)?state.proofs:[]).filter(p=>p.team===own).slice(0,100).map(p=>({id:String(p.id||""),tileIndex:Number(p.tileIndex),player:String(p.player||""),url:String(p.url||""),note:String(p.note||""),quantity:Math.max(1,Number(p.quantity)||1),status:String(p.status||"pending"),createdAt:String(p.createdAt||"")}))},{headers:noStore});}
