...
function completedTileCount(tiles,team){return (Array.isArray(tiles)?tiles:[]).reduce((total,tile)=>{const required=Math.max(1,Number(tile?.quantity)||1);const completed=Math.max(0,Number(tile?.teamProgress?.[team]?.completedQuantity)||0);return total+(tile?.name&&completed>=required?1:0);},0);}
